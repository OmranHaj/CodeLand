...
const LEVEL_ID = "cpp-logic-gates";
...
function saveProgress(userId, progress) {
  try {
    ...
    console.error(
      "[CodeLand] Unable to save C++ Logic Gates progress:",
      error,
    );
  }
}
...
async function runCppCode({
  ...
  return {
    ...
    error: null,
    message:
      "The CodeLand C++ compiler service is not connected yet.",
  };
}
...
function CppLogicGates() {
  ...
  const content = cppLogicGatesContent;
  ...
  const [xpToast, setXpToast] =
    useState(null);

  const [feedbackEffect, setFeedbackEffect] =
    useState(null);
  ...
  const isCompleted = (unit) => {
    ...
    if (unit.kind === "lesson") {
      return learningProgress.completedLessonIds.includes(
        unit.id,
      );
    }

    return learningProgress.completedChallengeIds.includes(
      unit.id,
    );
  };
  ...
  const completedCount =
    learningProgress.completedLessonIds
      .length +
    learningProgress.completedChallengeIds
      .length;
  ...
  const completeUnit = (unit) => {
    ...
    if (
      unit.kind === "lesson" &&
      !next.completedLessonIds.includes(
        unit.id,
      )
    ) {
      next.completedLessonIds.push(
        unit.id,
      );
    }
    ...
    if (
      unit.kind === "challenge" &&
      !next.completedChallengeIds.includes(
        unit.id,
      )
    ) {
      next.completedChallengeIds.push(
        unit.id,
      );
    }

    const nextCompleted =
      next.completedLessonIds.length +
      next.completedChallengeIds.length;
    ...
    try {
      ...
      if (response.passed) {
        setResult({
          ...
          passed: true,
          message:
            "Compilation successful. Mission complete!",
        });
  ...
  const robotMessage =
    robotReaction.type === "happy"
      ? "Excellent. Logic Gates are routing decisions correctly."
      : robotReaction.type === "sad"
        ...
        : runnerAvailable
          ...
          : "Logic Terminal is online. Backend compilation comes next.";
      ...
      {levelComplete && (
        ...
        >
          ...
          >
            ...
            <p>
              You brought the Logic Gates online. Your next chamber introduces functions, parameters, return values, scope, and reusable systems.
            </p>
